...
!******************************************************************
!
      real*8 function mGrdPntD2(lon,lat,dt,row,col,hd)
!利用距离平方反比方法内插(lon,lat)处的函数值
!2005年4月20日,章传银
!-------------------------------------------------------------
      implicit none
...
!-------------------------------------------------------------
	...
	ii=nint((lat-hd(3))/hd(6)+0.5d0)
	jj=nint((lon-hd(1))/hd(5)+0.5d0)
	qi=0.d0;dm=0.d0;mGrdPntD2=0.d0
      mdr=hd(5)*RAD*ae*1.d-3
	do ki=ii-5,ii+5
	  lat0=hd(3)+(real(ki)-0.5d0)*hd(6)
	  do kj=jj-5,jj+5
	    lon0=hd(1)+(real(kj)-0.5d0)*hd(5)
	    rr=((lon-lon0)*dcos(lat*RAD))**2+(lat-lat0)**2
	    rr=rr*ae**2+mdr**2
	    if(ki>0 .and. kj>0 .and. ki<=row .and. kj<=col)then
            if(dabs(dt(ki,kj))<9900.d8)then
	        qi=qi+1.d0/rr;dm=dm+dt(ki,kj)/rr
            endif
          endif
	  enddo
	enddo
	if(qi>1.d-16)mGrdPntD2=dm/qi
9100  continue
      end
